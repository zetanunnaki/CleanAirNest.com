import { getProduct } from "@/lib/products";
import { Button } from "@/components/ui/button";
import { RefreshCw, ExternalLink } from "lucide-react";

interface FilterReplacementCTAProps {
  productId: string;
}

export function FilterReplacementCTA({ productId }: FilterReplacementCTAProps) {
  const product = getProduct(productId);
  if (!product || !product.filterReplacementLink) return null;

  return (
    <div className="my-8 sm:my-10 border border-accent/20 rounded-2xl sm:rounded-[32px] p-4 sm:p-8 md:p-10 bg-gradient-to-br from-sky-50/80 to-white relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-accent/5 blur-[80px] rounded-full"></div>
      <div className="relative z-10 flex flex-col sm:flex-row items-start gap-3 sm:gap-6">
        <div className="w-10 h-10 sm:w-14 sm:h-14 bg-accent rounded-xl sm:rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-accent/20 animate-float">
          <RefreshCw className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="font-display font-bold text-base sm:text-xl text-slate-900 mb-1.5 sm:mb-2">
            Time for a Filter Replacement?
          </h4>
          <p className="text-slate-500 text-sm sm:text-base font-light leading-relaxed mb-4 sm:mb-5">
            The {product.name} needs a new filter{" "}
            <strong className="text-slate-900">{product.filterReplacementCycle?.toLowerCase() || "periodically"}</strong>.
            Genuine replacement filters maintain peak performance and keep your warranty intact.
          </p>
          <a
            href={product.filterReplacementLink}
            target="_blank"
            rel="noopener noreferrer nofollow"
            data-affiliate="amazon"
            data-product-id={productId}
            data-cta="filter-replacement"
            className="inline-block"
          >
            <Button variant="accent" size="lg">
              Buy Replacement Filter
              <ExternalLink className="w-4 h-4" />
            </Button>
          </a>
        </div>
      </div>
    </div>
  );
}
